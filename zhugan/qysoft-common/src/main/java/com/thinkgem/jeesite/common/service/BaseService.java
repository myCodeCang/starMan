	...
	public static String dataScopeFilter(User user, String officeAlias, String userAlias) {
		...
		if (!user.isAdmin()){
			...
			for (Role r : user.getRoleList()){
				for (String oa : StringUtils2.split(officeAlias, ",")){
					if (!dataScope.contains(r.getDataScope()) && StringUtils2.isNotBlank(oa)){
...
//							String officeIds =  StringUtils2.join(r.getOfficeIdList(), "','");
//							if (StringUtils2.isNotEmpty(officeIds)){
...
//							}
			...
			if (!isDataScopeAll){
				if (StringUtils2.isNotBlank(userAlias)){
					for (String ua : StringUtils2.split(userAlias, ",")){
		...
		if (StringUtils2.isNotBlank(sqlString.toString())){
	...
	public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String officeWheres, String userWheres) {
		...
		// 生成部门权限SQL语句
		for (String where : StringUtils2.split(officeWheres, ",")){
		...
		// 生成个人权限SQL语句
		for (String where : StringUtils2.split(userWheres, ",")){
