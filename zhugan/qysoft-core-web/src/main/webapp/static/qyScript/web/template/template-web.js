...
//格式化日期(截取掉日期)
    ...
    template.defaults.imports.timeFormat = function(date){
        ...
        var dates = date.substring(11,16);
        return dates;
...
template.defaults.imports.subPhoneFormat = function(phoneNumber){

	var phoneNumber = phoneNumber.substring(0,3)+"****"+phoneNumber.substring(8,11);
	return phoneNumber;
};

//银行卡隐私处理

    template.defaults.imports.subBankNumFormat = function(bankNumber){

        var bankNumber = bankNumber.substring(0,9)+"******"+bankNumber.substring(15,19);
        return bankNumber;
    };
...
//比较时间
//
//     template.defaults.imports.compareDate = function(date,date2){
