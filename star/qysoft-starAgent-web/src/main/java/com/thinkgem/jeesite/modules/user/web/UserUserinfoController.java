...
public class UserUserinfoController extends BaseController {
    ...
    public String userAgent(UserUserinfo userUserinfo, HttpServletRequest request, HttpServletResponse response, Model model) {
            userUserinfo.setIsShop("1");
            Page<UserUserinfo> page = userUserinfoService.findPage(new Page<UserUserinfo>(request, response), userUserinfo);
            model.addAttribute("page", page);
            return "modules/user/userAgentList";
    }
    ...
    public String form(UserUserinfo userUserinfo, Model model) {
        ...
        List<UserTeamLevel> userTeamLevels = userTeamLevelService.findList(new UserTeamLevel());
        model.addAttribute("userLevels", userLevels);
        model.addAttribute("userTeamLevels", userTeamLevels);
    ...
    public String save(UserUserinfo userUserinfo, Model model, RedirectAttributes redirectAttributes) throws ValidationException {
        ...
        try {
            UserUserinfo userinfo=userUserinfoService.get(userUserinfo.getId());
            if (userinfo!=null){
                if(Integer.parseInt(userinfo.getUserLevelId()) > Integer.parseInt(userUserinfo.getUserLevelId())){
                    addMessage(model, "会员等级不可降!");
                    return form(userUserinfo, model);
                }
                 UserUserinfo getParentObject = userUserinfoService.getByName(userinfo.getParentName());
                if(getParentObject==null){
                    addMessage(model, "该会员推荐人不存在!");
                    return form(userUserinfo, model);
                }
//                if(Integer.parseInt(getParentObject.getUserType())!=0){
//                    if(Integer.parseInt(userUserinfo.getUserType())!=Integer.parseInt(getParentObject.getUserType())-1){
//                            addMessage(model, "不可越级设置代理等级!");
//                            return form(userUserinfo, model);
//                        }
//                }else{
//                    if(Integer.parseInt(userUserinfo.getUserType())!=3){
//                        addMessage(model, "不可越级设置代理等级!");
//                        return form(userUserinfo, model);
//                    }
//                }
    ...
    public String savecenter(UserUserinfo userUserinfo, Model model, RedirectAttributes redirectAttributes) throws ValidationException {
        ...
        try {
            UserUserinfo user= userUserinfoService.get(userUserinfo.getId());
            if(user == null){
                throw new ValidationException("用户不存在");
            }
            userExStarService.backoutCenter(user, userUserinfo.getIsUsercenter());
    ...
    @RequiresPermissions("user:userUserinfo:edit")
    @RequestMapping(value = "updateIsShop")
    public String updateIsShop(UserUserinfo userUserinfo, Model model, RedirectAttributes redirectAttributes) throws ValidationException {
            String isShop;
            if("0".equals(userUserinfo.getIsShop())){
                isShop = "0";
            }else {
                UserUserinfo personInfo = userUserinfoService.getByName(userUserinfo.getUserName());
                if(EnumUtil.YesNO.YES.toString().equals(personInfo.getIsUsercenter())){
                    addMessage(model, "失败:"+"该用户已被指定为承销商!");
                    return userAgentform(userUserinfo, model);
                }
                if (!EnumUtil.YesNO.NO.toString().equals(personInfo.getUserType())){
                    addMessage(model, "失败:"+"该用户已被指定为代理!");
                    return userAgentform(userUserinfo, model);
                }
                isShop = "1";
            }
            userUserinfoDao.updateIsShop(userUserinfo.getUserName(),isShop);
            return "redirect:" + Global.getAdminPath() + "/user/userUserinfo/userAgent";
    }
    ...
    public String userCenterform(UserUserinfo userUserinfo, Model model) {
        UserTeamLevel userTeamLevel = new UserTeamLevel();
        userTeamLevel.setTeamCodeBegin("1");
        List<UserTeamLevel> userTeamLevelList =userTeamLevelService.findList(userTeamLevel);
        model.addAttribute("userTeamLevelList",userTeamLevelList);
        model.addAttribute("userUserinfo", userUserinfo);
        return "modules/user/userUserCenterForm";
    }

    @RequiresPermissions("user:userUserinfo:view")
    @RequestMapping(value = "userAgentform")
    public String userAgentform(UserUserinfo userUserinfo, Model model) {
        return "modules/user/userUserAgentForm";
    }
    ...
    @RecordLog(logTitle = "用户管理-承销商管理-指定承销商-新增")
    @RequiresPermissions("user:userUserinfo:edit")
    @RequestMapping(value = "updateUserTypeByName")
    public String updateUserTypeByName(UserUserinfo userUserinfo, Model model, RedirectAttributes redirectAttributes) throws ValidationException {
        UserUserinfo byName = userUserinfoService.getByName(userUserinfo.getUserName());
        if(!EnumUtil.YesNO.NO.toString().equals(byName.getUserType())){
            addMessage(redirectAttributes, "失败：该用户已被设置为代理,不可再指定为承销商!");
            return "redirect:" + Global.getAdminPath() + "/user/userUserinfo/list?isUsercenter=1";
        }
        if(EnumUtil.YesNO.YES.toString().equals(byName.getIsShop())){
            addMessage(redirectAttributes, "失败：该用户已被设置为经纪人,不可再指定为承销商!");
            return "redirect:" + Global.getAdminPath() + "/user/userUserinfo/list?isUsercenter=1";
        }
        if("1".equals(byName.getId())){
            addMessage(redirectAttributes, "失败：平台账户不可被设置为承销商");
            return "redirect:" + Global.getAdminPath() + "/user/userUserinfo/list?isUsercenter=1";
        }
        try {
            userExStarService.updateUserIsUsercenter(userUserinfo.getUserName(),byName.getShopId(),EnumUtil.YesNO.YES.toString ());
        } catch (ValidationException e) {
            addMessage(redirectAttributes, "失败："+e.getMessage());
            return "redirect:" + Global.getAdminPath() + "/user/userUserinfo/list?isUsercenter=1";
        }
        addMessage(redirectAttributes, "设置承销商成功!");
        return "redirect:" + Global.getAdminPath() + "/user/userUserinfo/list?isUsercenter=1";
    }
    /**
     * 导出用户数据
    ...
    public String userInfoExport(UserUserinfo user, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
        try {
            String fileName = "用户数据"+ DateUtils2.getDate("yyyyMMddHHmmss")+".xlsx";
            Page<UserUserinfo> page = userUserinfoService.findPage(new Page<UserUserinfo>(request, response, -1), user);
            new ExportExcel("用户数据", UserUserinfo.class).setDataList(page.getList()).write(response, fileName).dispose();
            return null;
        } catch (Exception e) {
     ...
     * @return
     */
    @RecordLog(logTitle = "系统设置-用户管理-升级代理等级")
    @RequiresPermissions("sys:user:view")
    @RequestMapping(value = "updateUserType", method=RequestMethod.POST)
    public String updateUserType(UserUserinfo user, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
        String  userType = request.getParameter("promValue");
        String  levelNumber = request.getParameter("levelNumber");
        if(!VerifyUtils.isNumeric(levelNumber)){
            addMessage(redirectAttributes, "失败：号段设置错误");
            return "redirect:" + Global.getAdminPath() + "/user/userUserinfo/list";
        }
        try {
            userExStarService.updateUserType(user.getId(),userType,levelNumber);
    ...
    public String check(UserWithdraw userUsercenterLog, String promValue, String promMsg, Model model, RedirectAttributes redirectAttributes){
        try {
            userExStarService.saveUserWithdraw(userUsercenterLog,promValue,promMsg);
            addMessage(redirectAttributes, "成功");
        } catch (ValidationException e) {
            addMessage(redirectAttributes, "失败:"+e.getMessage());
        } catch (Exception e){
            addMessage(redirectAttributes, "失败,打款异常");
        }
