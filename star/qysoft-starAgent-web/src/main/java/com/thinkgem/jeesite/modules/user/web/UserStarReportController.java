 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/user/userStarReport")
public class UserStarReportController extends BaseController {
	...
	public String starReport(UserReport userReport, HttpServletRequest request, HttpServletResponse response, Model model) {
		if(userReport.getCreateDateBegin()!=null || userReport.getCreateDateEnd()!=null){
			Page<UserReport> page = userReportService.sumStarReportPage(new Page<UserReport>(request, response),userReport);
			model.addAttribute("page", page);
		}
		...
		return "modules/user/userStarReportList";
	}
