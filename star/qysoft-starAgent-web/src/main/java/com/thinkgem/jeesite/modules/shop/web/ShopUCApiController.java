...
import com.thinkgem.jeesite.modules.user.service.UserEjfxService;
...
public class ShopUCApiController extends BaseController {
            ...
            if (syncScore.isPresent() && "on".equals(syncScore.get())) {
                ...
                if (syncScoreField.isPresent()) {
                    ...
                    if (EnumUtil.MoneyType.score.toString().equals(syncFile)) {
                        userUserinfoService.updateUserScore(userInfo.getUserName(), new BigDecimal(money), "商城积分消费" +
                                "", changeType);
                    } else {
    ...
    public String receiveOrder(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        try {
            ...
            //奖励积分
           // userUserinfoService.updateUserScore(userinfo.getUserName(),new BigDecimal(orderMoney),"商城购物赠送积分", EnumUtil.MoneyChangeType.ScoreForDay);
            //直推奖励
            userEjfxService.shopBuyBouns(mobile,orderMoney);
            //团队奖励
            userEjfxService.shopBuyTeamBouns(mobile,orderMoney);
            return success("成功!!", response, model);
        } catch (ValidationException e) {
