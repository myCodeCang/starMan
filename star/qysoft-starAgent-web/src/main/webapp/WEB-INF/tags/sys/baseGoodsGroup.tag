...
<div class="input-append">
	<input id="${id}Id" name="groupId" class="${cssClass}" type="hidden" value="${labelValue}"/>
...
<script type="text/javascript">
	$("#${id}").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/star/baseGoodsGroup/selectGoods?pageSize=8", "选择${title}", 750, 500, {
			ajaxData:{selectIds: ""},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				...
				if (v=="ok"){
					...
					var name = $(table).find(".selected").attr('data2');
					$("#${id}Id").val(id);
                    $("#${id}").val(name);
				}
				else if (v=="clear"){
